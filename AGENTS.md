...
**exeuresis** - A Python tool for extracting and reformatting Greek texts from TEI XML files in the Perseus Digital Library. The name comes from εὕρεσις (heuresis/exeuresis), meaning "discovery" or "finding out". Generates plain text versions in various styles (modern edition to ancient scriptio continua).
...
.venv/bin/python -m exeuresis.cli search "Plato"

# Extract text by work name or ID
.venv/bin/python -m exeuresis.cli extract euthyphro --style A
.venv/bin/python -m exeuresis.cli extract tlg0059.tlg001 --style A

# Extract specific range
.venv/bin/python -m exeuresis.cli extract euthyphro 2a-5e --print
...
### Catalog Browsing
```bash
.venv/bin/python -m exeuresis.cli list-authors              # All 99 authors
.venv/bin/python -m exeuresis.cli list-works Plato          # By author name (case-insensitive)
.venv/bin/python -m exeuresis.cli list-works tlg0059        # Or by TLG ID
.venv/bin/python -m exeuresis.cli list-works --all          # All 818 works
.venv/bin/python -m exeuresis.cli search "Republic"         # Search by title/author
```
...
.venv/bin/python -m exeuresis.cli extract euthyphro --passages 5a,7b-7c,10a

# Multiple works
.venv/bin/python -m exeuresis.cli extract euthyphro --passages 5a republic --passages 354b

# Using TLG IDs
.venv/bin/python -m exeuresis.cli extract tlg0059.tlg001 --passages 2a,3b tlg0059.tlg030 --passages 327a
```
...
**Alias Configuration**:
- User config: `~/.exeuresis/aliases.yaml`
- Project config: `.exeuresis/aliases.yaml` (overrides user)
...
.venv/bin/python -m exeuresis.cli extract tlg0059.tlg001

# Debug mode: full tracebacks and metadata parsing issues
.venv/bin/python -m exeuresis.cli --debug extract tlg0059.tlg001
```
...
```bash
# 1. Search for work
.venv/bin/python -m exeuresis.cli search "Phaedo"

# 2. Extract by work name alias (case-insensitive)
.venv/bin/python -m exeuresis.cli extract phaedo --style A

# 3. Or extract by TLG work ID
.venv/bin/python -m exeuresis.cli extract tlg0059.tlg004 --style A

# 4. Extract specific range with alias
.venv/bin/python -m exeuresis.cli extract phaedo 59a-60b --print
```

### Extract by File Path (backward compatible)
```bash
.venv/bin/python -m exeuresis.cli extract canonical-greekLit/data/tlg0059/tlg004/tlg0059.tlg004.perseus-grc2.xml
```
